    ...
    setExpandedObservations(prev => {
      ...
      apartmentList.forEach(apt => {
        if (apt.idApartamentoVistoria) {
          if (expand && apt.txObservacaoRevistoria) {
      ...
      <div className="space-y-6">
        {Object.keys(groupedApartments).length === 0 ? (
          ...
          Object.entries(groupedApartments).map(([dateKey, list]) => {
            ...
            const allAreExpanded = hasObservations && apartmentsWithObs.every(apt => expandedObservations.has(apt.idApartamentoVistoria!));
